// Diff Engine (named export + default export for compatibility)
export function computeDiff(previous, current) {
  ...
  for (const [id, role] of currRoles) {
    if (!prevRoles.has(id)) diff.roles.created.push(role);
    else {
  ...
  for (const [id, channel] of currChannels) {
    if (!prevChannels.has(id)) diff.channels.created.push(channel);
    else {
      const prev = prevChannels.get(id);
      const prevPerms = JSON.stringify(prev.permissionOverwrites ?? []);
      const currPerms = JSON.stringify(channel.permissionOverwrites ?? []);
      if (
  ...
  for (const [id, channel] of prevChannels) {
    if (!currChannels.has(id)) diff.channels.deleted.push(channel);
  }
